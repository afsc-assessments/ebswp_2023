...
{
  dvector temp1("{50,200,500,800,900,15000}");
...
void model_parameters::Recruitment_Likelihood(void)
{
  ...
  else
  {
    ...
    // if (ctrl_flag(30)==0)// use srr in fit (not just the prior) { }
      ...
      if (last_phase()) rec_like(1) *= ctrl_flag(30);
  }
...
void model_parameters::write_R(void)
{
 ...
 // get spr rates for ABC and OFL
  ...
  report << endyr_r+1<<" " << ABC <<" " << OFL <<" "<< future_SSB(6,styr_fut) <<" "<< age_3_plus_biom(endyr_r+1) <<" "<<
  catch_future(6,styr_fut) <<" " << hm_f  <<" "<< am_f <<" "<< gm_b(6) <<" "<< SPR_ABC <<" "<< SPR_OFL <<endl;
  ABC  = gm_b2(1)*hm_f*adj_2(1); 
  OFL  = gm_b2(1)*am_f*adj_2(1); 
  report << endyr_r+2<<" " << ABC <<" " << OFL <<" "<< future_SSB(6,styr_fut+1) <<" "<< age_3_plus_biom(endyr_r+2)  <<" "<<
  catch_future(6,styr_fut+1) <<" " << hm_f  <<" "<< am_f <<" "<< gm_b2(6)<<" "<< SPR_ABC <<" "<< SPR_OFL <<endl;
	report <<"YC"<<endl;
	int age1tmp=1990;
