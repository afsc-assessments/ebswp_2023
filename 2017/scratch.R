...
#--- get M profile------
for (i in 10:30) {
  rn=paste0("16.0/arc/r_R",i,".rep")
  mn=paste0("m",i)
  assign(mn,readList(rn))
  print(rn)
}

#---plot up new MACE ats age data----------------
library(tidyverse)
adf <- read_csv("data/mace_ages.csv")
adf.g <- gather(adf,key=Age,value=N,-Coverage,-Year)
adf.g
library(ggridges)
ggplot(adf.g,aes(x=as.numeric(Age),y=as.factor(Year),height = log(N),fill=Coverage,color=Coverage)) + geom_density_ridges(stat = "identity",scale = 1.2, alpha = .1)+ xlim(c(1,11))+ mytheme + ylab("Year") + xlab("Age (years)") #+ theme_ridges()  
ggplot(adf.g,aes(x=as.numeric(Age),y=as.factor(Year),height = (N),fill=Coverage,color=Coverage)) + geom_density_ridges(stat = "identity",scale = 1.2, alpha = .1)+ xlim(c(1,11))+ mytheme + ylab("Year") + xlab("Age (years)") #+ theme_ridges()  
M <- readList("16.0/For_R.rep")
Mly <- readList("arc/16.1/For_R.rep")

#---Selectivity
df <- data.frame(Year=M$Yr,sel=M$sel_fsh[,1:15] ); 
names(df) <- c("Year",1:15)
dim(sdf)
(sdf)
sdf <- gather(df,age,sel,2:16,-Year) %>% filter(Year>1976) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
...
ggplot(sdf,aes(x=age,y=as.factor(Year),height = sel)) + geom_density_ridges(stat = "identity",scale = 3.8, alpha = .2,fill="salmon") + 
               xlim(c(1,15))+ mytheme + ylab("Year") + xlab("Age (years)") + 
               scale_y_discrete(limits=rev(levels(as.factor(sdf$Year))))
ggsave("../doc/figs/fsh_sel.png")
 
#---Fit to age data
pdf("../doc/figs/fsh_age.pdf",width=6,height=8)
AgeFits(labrep.file="16.0/For_R.rep",case_label="2017 Assessment")
dev.off()
modsc$FW_fsh
mod16.02$FW_fsh
...
mod0.0 <- readList("m0/For_R.rep")
mod16.1 <- readList("arc/16.1/For_R.rep")
mod15.1 <- readList("arc15.1/For_R.rep")

modlst <- M
list("17.0"= mod0.0,  "16.1 2016"=mod16.1)

modlst <- list("15.1 2015"=mod0.0, "15.1 2016"=mod15.1,"16.1"=mod16.1  )
modlst <- list("16.1"=mod16.1,  "15.1 2016"=mod15.1,"15.1 2015"=mod0.0) 
modlst <- list("This year"=M,"Last year"=Mly)
plot_srr(modlst,alpha=.4,xlim=c(0,5000),ylim=c(0,65000))

plot_ssb(modlst,xlim=c(2005,2017),alpha=.3)
plot_recruitment(modlst,xlim=c(1963.5,2016.5))

library(PBSadmb)
mod16.0  <- readList("../runs/16.1/For_R.rep")
mod16.1  <- readList("../runs/arc/16.1/For_R.rep")
mod16.1a <- readList("../runs/arc/16.1a/For_R.rep")
mod16.1b <- readList("../runs/arc/16.1b/For_R.rep")
...
AgeFits(M,case_label="2017 Model 16.0")
...
avo <- data.table(pred=M$pred_avo,obs=M$obs_avo,yr=2006:2017)
ggplot(avo,aes(x=yr,y=obs)) + geom_errorbar(width=.2,ymin=avo$obs-avo$obs*.25,ymax=avo$obs+avo$obs*.25) + geom_line(aes(x=avo$yr,y=avo$pred)) + .THEME + geom_point(size=4,col="red") + xlab("Year") + 
             scale_x_continuous(breaks=seq(2006,2016,1)) + ylab("AVO index") + ylim(c(0,1.5))

getwd()
at <-read.table("~/_mymods/ebswp/at_srv.dat",header=T)
...
dt <- data.table(Year=M$yr_bts,Observed=M$ob_bt, Predicted=M$eb_bt, sd=M$sd_ob_bts)
names(M)
dtm<-data.table(melt(dt,id="Year")) 
(dtm)
ggplot(dtm,aes(x=Year,y=value,col=variable)) + geom_point(data=dt,aes(x=Year,y=Observed,size=3)) + 
    geom_line(data=dtm[variable=="Predicted"]) + geom_errorbar(data=dt,ymin=dt$Observed-dt$sd ,ymax=dt$Observed+dt$sd ) + ylim(c(0,14000)) + labs(x="Year",y="Survey biomass") + mytheme #+
+ geom_errorbar(width=.2,ymin=avo$obs-avo$obs*.25,ymax=avo$obs+avo$obs*.25) + geom_line(aes(x=avo$yr,y=avo$pred)) + .THEME + geom_point(size=4,col="red") + xlab("Year") + 
#-------------------------------------------------------------------------------
#Selectivity
IndexFit(mod16.02,yf=1980,yl=2015,f=1,main="Model 1",ylab="Survey biomass (t)")
IndexFit(M,yf=1980,yl=2015,f=1,main="Model 5",ylab="Survey biomass (t)")
...
AgeFits(M,rec_age=1,case_label="Model 6 assessment")
...
AgeFitsSrv(M,rec_age=1,case_label="2015 assessment")
Plot_Fspr()
...
for (i in 0:20) {
  ...
  names(retouts)
plot_retro(retouts)
plot_ssb_rel(retouts,xlim=c(1980,2017),ylim=c(0.5,1.5),legend=FALSE,alpha=.2)
