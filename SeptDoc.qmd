...
\hline
Base                           & base22 &                 & Original from folder 07, now in folder base22 with new code that accommodates FTNIR and GenGamma                                                                                                                                            \\
\hline 
...
\hline 
Alternative wt-age             & ssb0   & base22          & Impact on SSB, consider variability by new wt-age estimates, but start w/ constant mean wt-age                                                                                                                                              \\
for SSB                        & ssb1   & base22, ssb     & Use raw means from A-season fishery as a proxy                                                                                                                                                                                              \\
...
These models are contrasted and categorized by the type of analysis involved. Specifically, we start with the 2022 assessment model and data configuration, then apply the revised new AVO series (AVON; @fig-schema). Here we assumed that the application of the better time series would be taken as a given. The next step in the model process was to re-evaluate the relative weights specified as input variances and sample sizes. We attempted to apply different tuning approaches which acheived a balance between the observation errors and model process errors. We then have a section where we did sensitivities of different assumptions to arrive at a condidate proposed model for the 2023 assessment.

![Schematic of approach for evaluating some planned changes in the EBS pollock assessment.](doc/figs/Schema.png){#fig-schema}
...
Given that we elect to use the latest AVO revised data, the next task was to evaluate alternative "tuning" approaches. As noted above, this entailed examining tradeoffs between observation and process error assumptions. Selectivity at age
...
In @fig-sel_ats_tuned we show how the alternative tuning approaches affected the relative fits and uncertainty assumptions. For the observation-error approach
...
was based on the A-season fishery, the summer survey data (scaled to have the 
same mean as the A-season means) shows distinct year and cohort effects (@fig-ssbwt). 
@fig-wt_ssb shows how the patterns differ Compared to the values used in the base model, 
the trends are very similar but the scales change substantially in some years 
(@fig-ssbssb ; including the "SSB0" scenario).
...
In this section we present the updated error terms associated with the acoustic 
trawl data (the observations used previously were revised). We also show the 
impact of using the diagonal covariance matrix and an alternative for the bottom 
trawl survey data based on the generalized gamma distribution in place of the log-normal.

The spawning biomass estimates @fig-diagssb ...xxx
...
-   adopting the use of the full revised AVO time series. *the data have been re-calibrated to the acoustic trawl survey and cover a larger area than previously. This may improve the ability to track expansion and contraction of the pollock stock in mid-water.*
...
-   pursuing model configuration that can best be used to bridge between other software platforms (e.g., WHAM, stock synthesis, and AMAK2)
...
References to add @oleary2022 @cheng2023 @maunder2020 @siskey2022 @indivero2023 @bailey2000
