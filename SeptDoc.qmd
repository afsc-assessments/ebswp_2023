...
then apply the revised new AVO series (AVON). We then applied the different
tuning approaches.
...
In  @fig-mod_index_fits_tuned we show how the alternative tuning approaches affected the relative fits and uncertainty assumptions. For the observation-error
tuning, the AVO error bars increased slightly while the opposite occurred for the ATS index.
