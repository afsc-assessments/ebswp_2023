...
d<-m <- 'pm' # the model name, folder is also assumed to be called this runs/base/mcmc
## First optimize the model to make sure the Hessian is good.
setwd(d); 
system('pm -nox -mcmc 15 -hbf 1 -ainp pm.par -phase 50'); setwd('..')


## Run parallel RWM chains as a first test
thin <- 200
iter <- 2000*thin; warmup <- iter/8
inits <- NULL ## start chains from MLE
?sample_admb
 pilot2 <- sample_admb(m, iter=iter, thin=thin, seeds=seeds, init=inits,
...
setwd(d); 
system(paste(m, '-hbf 1 -nox -mcmc 15')); 
setwd('..')
getwd()
## Use default MLE covariance (mass matrix) and short parallel NUTS chains
## started from the MLE.
nuts.mle <- sample_admb(model=m, iter=2000, init=NULL, algorithm='NUTS', thin=2,  seeds=seeds,
              parallel=TRUE, chains=reps, warmup=400, cores=reps,
              control=list(metric="mle", adapt_delta=0.9))
nuts2.mle <- sample_admb(model=m, iter=800, init=NULL, algorithm='NUTS', thin=1,  seeds=seeds,
              parallel=TRUE, chains=reps, warmup=200, cores=reps,
              control=list(metric=nuts.mle$covar.est , adapt_delta=0.98))
