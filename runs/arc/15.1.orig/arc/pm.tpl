...
DATA_SECTION
 ...
 !! ad_comm::change_datafile_name(datafile_name);
  ...
  !! write_log(oac_bts_data); write_log(std_eit);
  ...
  !! write_log(oac_eit_data); write_log(std_eit);
  init_vector obs_eit_data(1,n_eit)
  init_vector std_obs_eit_data(1,n_eit)
  !! write_log(obs_eit_data); write_log(std_obs_eit_data);
 ...
 LOCAL_CALCS
  ...
  if (phase_sr>0) 
    ...
    phase_nosr=1;
 END_CALCS
  // !! ad_comm::change_datafile_name("cov.dat");
...
PARAMETER_SECTION
 ...
 // Average weight stuff
  matrix wt_pre(styr_wt,endyr_wt+3,age_st,age_end)
  init_vector mnwt(age_st,age_end,1);
...
FUNCTION GetDependentVar 
  ...
  if (sd_phase()||mceval_phase())
  {
    ...
    {
      res.initialize(); 
      sel_fut = sel_fsh(endyr-iyr+1);
      sel_fut/=sel_fut(6); // NORMALIZE TO AGE 6
      res = get_msy_wt();
      Fmsy2_dec(iyr) = res(4); 
      cout <<endyr - iyr +1<<" "<<res<<endl;
    }
    
    for (int iyr=10;iyr>=1;iyr--)
    {
      res.initialize(); 
      wt_fut(3,nages) = wt_pre(endyr-iyr+1);
      res = get_msy_wt();
