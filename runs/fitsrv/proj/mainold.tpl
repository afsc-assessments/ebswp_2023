...
FUNCTION Run_Sim
  detail_out<<"Alternative SpNo Spp Yr  ABC OFL Catch SSB F Tot_biom SPR_Implied Ntot SexRatio FABC FOFL"<<endl;
    for (int ispp=1;ispp<=nspp;ispp++)
    {
   ...
   { // call and get TAC's fo this species in this year...
     ...
     for (int ispp=1;ispp<=nspp;ispp++) 
       detail_out<<alt<<" "<<ispp<<" "<<spname(ispp)<<" "<<styr+ipro-1<<" "<<ABC(ispp)<<" "<<OFL(ispp)<<" "<<Actual_Catch(ispp)
                      ...
                      <<" "<<Nsim(ispp,isim,ipro) <<" "<<SRsim(ispp,isim,ipro)<< " "<< FABCs_by_yr(ipro,ispp)<<" "<<FOFLs_by_yr(ipro,ispp)<<endl;
   } // Loop over projection years
