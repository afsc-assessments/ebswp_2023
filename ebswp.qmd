---
title: "Stock assessment for eastern Bering Sea walleye pollock"
author: 
 -  name: James Ianelli 
    corresponding: true
    id: ji
...
a northern extension region north of the core shelf survey area. The BTS
chartered boats also collected acoustic data and the series was updated
...
5.  We revised applied a new time series from the acoustic data
...
series. We examined additional model sensitivities under development.
Finally, we adopted alternative estimates of weight-at-age applied to
...
SSC agreed with the data / model changes and those present the new base case 
presented here.
...
area covered by acoustics and provided more precision in trend than in the 
past. Ancilliary data indicate that the pollock in 2023 are substantially
...
-   *For the 2023 assessment we examine the variability of the
    ...
    cthealculations can result in highly variable estimates as the stock
    approaches $B_{MSY}$ and drops below that value (as happend in the
    2009-2010 period).
...
@buckley2015 showed spatial patterns of pollock foraging by size of
...
piscivorous than counterparts in other areas.

## Stock structure

Stock structure for EBS pollock was evaluated in @ianelli2015 Ianelli et
al. (2015). In that review past work on genetics (e.g., Bailey et al.
...
see appendix for details) are shown in @fig-bts_biom. In the
...
such center of gravity estimates indicate that the stock has moved steadily north since mid 2000s 
but last year shifted fairly south. This year it has shifted back north to some degree (@fig-cog). 
...
2022, but still represents the most abundant. The abundance of age-1 pollock in 
...
VAST--see appendix for those details). Compared to the previous
...
stations), in 2019, 2021-2023. Given that the pollock
...
approach for constructing a full time series that included the NBS area
...
\cref{tab:btsabund}). Please refer to the appendix for further details
...
abundance and distribution were assessed from Bristol Bay to the
U.S.-Russia Convention Line from 1 June to 5 August 2022 using
acoustic-trawl (AT) survey methods aboard the NOAA ship *Oscar Dyson*.
In addition to the traditional (core) survey area, a region north of
most transects (the northern extension) was surveyed. Transect spacing,
typically 20 nmi, ranged from 40 nmi in the east and middle shelf to 20
nmi in the western shelf due to ship staffing constraints and
uncertainties over the survey schedule.

The estimated amount of pollock in midwater (between 16 m from the sea
surface and 0.5 m off the sea floor) in the U.S. EEZ core survey area
was 9.67 billion fish with a biomass of 3.834 million metric tons (t),
just over a 50% increase from the estimate of 5.57 billion fish with a
biomass of 2.499 million t in 2018, the last time a standard
acoustic-trawl survey was conducted on the Eastern Bering Sea shelf.
This is a 6% increase over the estimate of 3.605 million t in 2020 made
by the acoustics-only Saildrone survey. Pollock east of 170\^\circ\$W
numbered 2.99 billion fish and weighed 1.361 million t (36% of the total
biomass found in the core survey area, and 31% of the biomass found
shelf-wide, including the northern extension). Pollock abundance west of
170W was 6.68 billion fish weighing 2.473 million t (64% of total core
survey area biomass and 57% of the shelf-wide biomass). Preliminary age
results using age-length information from the BT survey and 100 AT
survey juvenile ages indicated that four year-old pollock (34 to 40 cm
...
assessment model. The age composition data from the ATS sampling are
provided in \cref{tab:atsage}).
...
in @low1980. A coefficient of variation of 20% was applied.

### Biomass index from Acoustic-Vessels-of-Opportunity (AVO)
Acoustic backscatter data (Simrad ES60, 38 kHz) were collected aboard
two fishing vessels chartered for the AFSC summer 2023 bottom trawl
surveys (F/V Alaska Knight, F/V Northwest Explorer). We process these
Acoustic Vessels of Opportunity (AVO) data each year to provide an index
of age-1+ midwater pollock abundance. This year, based on implementing a
new subsampling methodology (@levine2019) to generate a more extensive
(spatially) AVO index. In developing the index, we applied a 10%
systematic subsample of the backscatter data throughout the biennial
acoustic-trawl (AT) survey footprint and matched that with analogous
acoustic samples collect from the BTS.

Relative to the previous index, the correlation between the AVO index
and the AT survey biomass is higher (R^2^ = 0.9 compared to that of R^2^
= 0.6 from the earlier studies of @honkalehto2011, @stienessen2020). The index trend
dropped 15% relative to 2022 but still is 13% above the long-term mean
(@fig-avo, \cref{tab:avo}; note that the relative error is based on a
variance estimation from @petitgas1993 while the final magnitude of the
error term was ascertained based on other model components via a
iterative re-weighting process as noted in @ianelli2023). The densest
spatial distribution of pollock backscatter was predominantly measured
along the southern portion of the EBS shelf in the northwest half of the
index area @fig-data_avomap). The three grid cells (20 by 20 nautical
mile grids used for annual bottom-trawl survey stations) having the
strongest pollock backscatter were 2^o^south of St. Matthew Island close
to (58^o^N, 172^o^W) and attributed to dense midwater pollock
aggregations.
...
presented in the "EBS Pollock Model Description" appendix. @ianelli1998.
...
-   The `r thisyr` bottom-trawl survey index, weight, and age
    composition data were revised and added
...
(\cref{tab:inputn}). We assumed average values of 100 and 50 for the BTS
and ATS data, respectively. The inter-annual variability reflects the
...
originally based on @smith1981 and were later reevaluated (e.g.,
@stahl2004; @Stahl2008, @ianelli2005). These studies found inter-annual
...
fits are given in \cref{tab:modfits} and comparisons of management
quantities are given in \cref{tab:mgtquants}).
...
dropped by over 10% and became the 2012 *and* the 2013 year-class. Given that
there is more confirmation that the 2018 year class may persist as being exceptionally
big. In the 2022 assessment we adjusted the value downwards to be equal to the 
mean of some earlier year classes. This year, we simply accepted the estimate
for projections.

Related to this issue, and in response to an SSC request, we evaluated the influence 
...
variability of the estimated curve (@fig-mod_srr). Note that the 2020
and 2021 year classes (as age 1 recruits in 2021 and 2022) were excluded
from the stock-recruitment curve estimation. Separate from fitting the
stock- recruit relationship within the model, examining the estimated
...
`r thisyr-1` $F_{OFL}$ from this year's model was estimated to be xx0.5431xx
for EBS pollock (assuming this year's estimated 2023 selectivity and
weight-at-age).
...
**We therefore rated the population-dynamics concern as level 1, Normal:
No apparent environmental/ecosystem concerns.**

#### Environmental/Ecosystem considerations

**Summary for Environmental/Ecosystem considerations**
The following summarizes “Environmental/Ecosystem” considerations (see appendix
for details):
...
warrants a score of 1, normal: no apparent fishery performance
concerns.**
...
indicators are provided in @tbl-dectable. Each column of this
...
in a catch of `r M$abc1constFs` kt. Given the revisions to last year's
...
\cref{tab:ecosystab}). Unlike the food-web models discussed above,
...
taken incidentally varies considerably over time. increased since 2014
...
from 2011-2022 (\cref{tab:pscbycatch}). After a recent high bycatch of
over 32,000 fish in 2020, the 2022 bycatch was just over 6,400 Chinook
salmon. @ianelli2014 provided estimates of the bycatch impact on Chinook
...
tab <- xtable(df, caption = tabcap[27], label = "tab:mgtquants", digits = 3,align=paste0("ll",strrep("r",length(mod_scen))))
...
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x}, scalebox=.70,comment=FALSE)
```
...
df <- data.frame(rownames(df), df, row.names = NULL)
names(df) <- c("Component",mod_names[mod_scen])
 tab <- xtable(df, caption = "Summary of model results and the stock condition for EBS pollock. Biomass units are thousands of t.", label = "tab:ressumm", digits = 3,align=paste0("ll",strrep("r",length(mod_scen))) )
...
#| output: asis
#| echo: false
#byc <- read.csv("doc/data/byc_PSC.csv",header=T)
#for (i in 2:length(byc[1,]) ) 
  #byc[,i] <- formatC(byc[,i], format="d", big.mark=",") 
#cap <- tabcap[39]
#tab <- xtable(byc, caption = cap, label=paste0("tab:",tablab[36]),digits=2, auto=TRUE, align=rep("r",(length(byc[1,])+1)) )
...
#byc <- read.csv("doc/data/byc_PSC.csv",header=T,as.is=TRUE)
#for (i in 2:length(byc[1,]) ) 
  #byc[,i] <- formatC(byc[,i], format="d", big.mark=",") 
#cap <- tabcap[39]
