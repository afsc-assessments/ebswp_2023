...
#---
  ...
  p1 <- plot_recruitment(modlst[c(1,2,3)],xlim=c(2008.5,2019.5));p1
  p2 <- plot_ssb(modlst[c(1,2,3)],xlim=c(2008.5,2020.5),breaks=seq(2008,2020,by=2),alpha=.2); p2
  # Comparing base with 2 vast configurations
  p3 <- arrangeGrob(p1,p2,nrow=2); p3
  ggsave("figs/mod_eval0a.pdf",plot=p3,width=8,height=5.0,units="in")
  # Comparing base with 2 vast configurations
  #p1 <- plot_recruitment(modlst[c(2,4,5)],xlim=c(2010.8,2019.5));p1
  p1 <- plot_bts(modlst[c(2,5)],xlim=c(1981.5,2019.5),ylim=c(0,30000)) 
...
p1 <- ggplot(df,aes(x=Year,y=SSB,ymax=ub,ymin=lb)) + geom_ribbon(fill="salmon",alpha=.6) + geom_line() +  theme_few() + 
  ...
  geom_vline(xintercept=2019,col="grey"); p1
  ...
  #---R/S------------------
  dt <- data.table(yr=M$SSB[1:55,1],ssb= M$SSB[1:55,2], r=M$R[2:56,2] ) 
  ...
  p1 <- plot_ser(modlst[thismod],xlim=c(1964,2020),alpha=.7)
  ...
  plot_agefit(M,case_label="2019 Assessment",gear="bts",type="survey")
  ...
  names(dd)
  names(dd) <- c("Year",2018:2006,2001:1998)
  head(dd.g)
  dd.g <- pivot_longer(dd,cols=2:18,names_to="Assessment",values_to="Biomass")
  # this line to add current year estimate!!
  t <- data.frame(Year=1964:2020, Assessment=2019,Biomass=c(M$age3plus,M$age3plus1)  )
  dd.g <- rbind(dd.g,t) 
  ...
  p1 <- ggplot(dd.g,aes(x=Year,y=Biomass,color=Assessment)) + geom_line(alpha=.8,size=.75) + .THEME + 
    scale_x_continuous(breaks=seq(1980,2020,5),limits=c(1980,2020))  +  xlab("Year") + ylim(0,14000) + ylab("Age 3+ biomass (kt)") +
  ...
  p1 <- plot_ssb(retouts,xlim=c(1990,2019),legend=F)
  p2 <- plot_ssb_rel(retouts,xlim=c(1990,2019),ylim=c(0.5,1.5),legend=F,alpha=.2)
  p3 <- arrangeGrob(p1,p2,nrow=2)
  print(p3)
  ggsave("figs/mod_retro.pdf",plot=p3,width=7.2,height=9.0,units="in")
  p1 <- plot_R_rel(retouts,xlim=c(1990,2018),legend=F,rel=FALSE)
  p2 <- plot_R_rel(retouts,xlim=c(1990,2018),ylim=c(0.5,1.5),legend=FALSE,alpha=.2)
  p3 <- arrangeGrob(p1,p2,nrow=2)
  ...
  pt <- df %>% filter(Year>=lastyr)%>% mutate(F.Fmsy=meanF/Fmsy,B.Bmsy=SSB/Bmsy,Year=substr(as.character(Year),3,4))  
  p1 <- df %>% filter(Year>1977) %>% mutate(F.Fmsy=meanF/Fmsy,B.Bmsy=SSB/Bmsy,Year=substr(as.character(Year),3,4)) %>% 
      ggplot(aes(x=B.Bmsy,y=F.Fmsy,label=Year)) + 
         geom_text(aes(color=as.factor(Year),size=24),col="blue",check_overlap = TRUE) + .THEME + xlim(c(0,2.2))+ ylim(c(0,1.1)) + xlab("B/Bmsy") + ylab("F/Fmsy") +
         geom_label(data=pt,size=8,fill="yellow",color="red",alpha=.4) +
