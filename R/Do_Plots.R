...
#  names(M)
 ...
 # p1 <- plot_mnage(modlst[c(2,3)]) 
  ...
  p1 <- plot_ats(modlst[c(1,2)]) ;p1
  ggsave("figs/mod_ats_bridging.pdf",plot=p1,width=5.2,height=3.7,units="in")
  p1 <- plot_bts(modlst[thismod],xlim=c(1980,2022)) ; p1
...
#  p1 <- p1+ geom_vline(xintercept=2006.5,color="grey",size=1)
  ...
  CAB_names <- (c("last year", "Catch-age added, 2020", "Add BTS data", "Add AVO"))
...
#  CAB_names <- c("last year", "Catch added","Agin")
  ...
  names(A)
  A$Model <- factor(A$Model ,levels= c("last year", "Catch-age added, 2020", "Add BTS data", "Add AVO"))
  p1 <- ggplot(A,aes(x=year,y=ssb,ymin=lb,ymax=ub,fill=Model)) + labs(x = "Year", y = "Spawning biomass") + .THEME +
        ...
        scale_x_continuous(limits=c(2009,2021), breaks=2009:2021)  +
...
p1 <- ggplot(df,aes(x=Year,y=SSB,ymax=ub,ymin=lb)) + geom_ribbon(fill="salmon",alpha=.6) + geom_line() +  theme_few() + 
  scale_x_continuous(limits=c(2000,2026),breaks=seq(2000,2026,2)) + 
  ylab("Female spawning biomass (kt)") + 
  geom_vline(xintercept=2021,col="grey"); p1
  ...
  p1 <- ggplot(td.g,aes(x=YEAR,y=temp,col=Source,shape=Source)) + geom_point(size=2) + geom_line(size=1.0) + .THEME + ylab("Degrees C") + expand_limits(y=0) + 
       scale_x_continuous(breaks=seq(1982,2021,3)) + geom_hline(yintercept=mst,linetype="dashed") + geom_hline(yintercept=mbt,linetype="dashed") 
       p1
  ...
  head(dd)
  names(dd) <- c("Year",2020:2006,2001:1998)
  ...
  p1 <- ggplot(dd.g,aes(x=Year,y=Biomass,color=Assessment)) + geom_line(alpha=.8,size=.75) + .THEME + 
    scale_x_continuous(breaks=seq(1980,thisyr+1,5),limits=c(1980,thisyr+1))  +  xlab("Year") + ylim(0,16000) + ylab("Age 3+ biomass (kt)") +
...
cd %>% filter(Area=="BSAI") %>% mutate(Subarea=ifelse(NMFS_area==518,"Bogoslof",
                                               ifelse(NMFS_area<520,"SE",
                                               ifelse(NMFS_area<540,"NW","Aleutians")))) %>% 
group_by(Year,Subarea) %>% summarise(catch=sum(catch)) %>% 
pivot_wider(names_from=c(Subarea),values_from=catch) %>% xtable() %>%  print(n=Inf)
