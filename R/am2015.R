...
reposDir    <- "/Users/jim/Onedrive/Models/Atka/2015"
...
outdir  = reposDir
Figdir  = paste0(reposDir,"/figs")
ret_dir = paste0(reposDir,"/retro")
codePath
setwd(codePath)
getwd()
...
dat_name="am2015.dat"
amak.in
amak.in  <- read.dat(iFilename = dat_name,iPath=paste0(inputPath,"/"))
amak.out <- readList(file.path(inputPath,paste("arc/",run_name,"_R.rep",sep="")))
mod2014 <- readList(file.path(inputPath,paste("arc/2014/",run_name,"_R.rep",sep="")))
...
df2 <- data.frame(mod2014$SSB )
names(df2) <- c("yr","SSB","SE","lb","ub")
bdf <- filter(df,yr>1977,yr<=2016)
bdf2<- filter(df2,yr>1977)

# try over same time range...
p1 <- ggplot() + scale_y_continuous(limits=c(0,590000)) + ylab("Spawning biomass") + xlab("Year") +  mytheme + geom_line(data=bdf,aes(x=yr,y=SSB)) +
           geom_ribbon(data=bdf ,aes(x=yr,y=SSB,ymin=lb,ymax=ub),fill="yellow",col="grey",alpha=.2)  + guides(fill=FALSE,alpha=FALSE,col=FALSE) 
p1 <- p1 + geom_ribbon(data=bdf2,aes(x=yr,y=SSB,ymin=lb,ymax=ub),fill="blue",col="grey",alpha=.2) + geom_line(data=bdf2,aes(x=yr,y=SSB),col="blue")
p1
#Recruits==========================================================
...
rdf  <- filter(rdf , yr>1976,yr<2015)
mnR <- mean(rdf$R)
mnR
...
tbl_df(rdf)
dodfact=0.90
ggplot(rdf,aes(x=as.factor(yr-1),y=R,group=case)) + xlab("Year class") + ylab("Age 1 recruits (10e6)") + 
...
#Selectivity==============================================
df <- data.frame(mod1$sel_fsh_1[,2:13] )
names(df) <- c("yr",1:11)
sdf <- gather(df,age,sel,2:12)
tbl_df(sdf)
sdf
p1 <- dplyr::filter(sdf,yr<1998) %>% arrange((yr)) %>%ggplot(aes(x=age,y=sel/2+yr,group=yr)) + geom_ribbon(aes(ymin=yr,ymax=sel/1.9+yr),fill="tan",col="grey60",alpha=.3)  + ylab("Selectivity by year") + xlab("Age") + guides(fill=FALSE,alpha=FALSE,col=FALSE) + mytheme
p2 <- dplyr::filter(sdf,yr>1997) %>% ggplot(aes(x=age,y=sel/2+yr,group=yr)) +                  geom_ribbon(aes(ymin=yr,ymax=sel/1.9+yr),fill="tan",col="grey60",alpha=.3)  + ylab("")                        + xlab("Age") + guides(fill=FALSE,alpha=FALSE,col=FALSE) + mytheme
...
#pdf(paste(Figdir,"Retro_Mods.pdf",sep=""),idth=9, height=6)
library(grid)
tdf <- data.frame(retro0$SSB )
names(tdf) <- c("yr","SSB","SE","lb","ub")
dim(tdf)
bdf <- filter(tdf,yr>=1977,yr<2016)
bdf2<- filter(tdf,yr>1977,yr<2016)
bdf
p1 <- ggplot() + scale_y_continuous(limits=c(0,590000)) + ylab("Spawning biomass") + xlab("Year") +  mytheme + 
           geom_ribbon(data=bdf ,aes(x=yr,y=SSB,ymin=lb,ymax=ub),fill="tan",col="grey",alpha=1)  + geom_line(data=bdf,aes(x=yr,y=SSB),size=1.2) +guides(fill=FALSE,alpha=FALSE,col=FALSE) 
for (i in 1:10) {
  rn=paste("retro",i,sep="");
  tdf <- data.frame(get(rn)$SSB); names(tdf) <- c("yr","SSB","SE","lb","ub"); tdf <- filter(tdf,yr>1977,yr<2016-i)
  p1 <- p1 + geom_line(data=tdf,aes(x=yr,y=SSB),col=i,size=.5)
  tdf <- tdf[dim(tdf)[1],]
...
bdf
bdft <- bdf
for (i in 1:15) bdft <- cbind(bdft,rep(NA,39))
names(bdft)[6:15] <- paste0("SSB_",2014:2005)
for (i in 1:15) bdft[1:(39-i),i+5] <- get(paste0("retro",i))$SSB[13:(51-i),2]
tbl_df(bdft)
tail(bdft)
...
for (i in 1:10) {
  ...
  names(tdf) <- c("yr", "SSB")
  p2 <- p2 + geom_line(data=tdf, aes(x=yr,y=SSB,size=1),col=i,size=.5)
  tdf <- tdf[dim(tdf)[1],]
...
for (i in 1:10) {
  ...
  print(paste(i,lr,rho/i,dtmp[lr]))
}    
dev.off()
(retro1$SSB[13:51,])
#============================================================kkkkkkkkkk
...
mod2014 <- readList("../arc/2014/mod1_R.rep")
...
thislast  <- list( "Model 1 this year"= mod1, "Last year's model"= mod2014)
thislast  <- list( "Model 1 this year"= mod1, "Last year's model"= mod2014)

# used in doc to compare this year w/ last...
compareTime(thislast,"SSB",SD=T,Sum=NULL,legendPos="topleft",startYear=1977,xlim=c(1977,2015),ylim=c(0,500000),main="Spawning biomass")
compareTime(thislast,"R",SD=T,legendPos="topright",xlim=c(1977,2015),ylim=c(0,2800))
...
compareTime(thislast,"SSB_NoFishR",SD=T,Sum=NULL,startYear=1977,ylim=c(0,1.0))
...
rec_age=1
p.rec.hist(mod1,main="Model 1",ylab="Recruitment to age",fy=1977,ly=2015,plotmean="T")
p.rec.hist(mod2014,main="2014 model",ylab="Recruitment",fy=1977,ly=2015,plotmean="T")
# Survey fit
...
# Likelihood table
tab       <- cbind(thislast[[1]]$Like_Comp_names,do.call(cbind,lapply(thislast,function(x){round(x[["Like_Comp"]],2)})))
tab
do.call(cbind,lapply(thislast,function(x){round(x[["Like_Comp"]],2)}))
do.call(cbind,lapply(thislast,function(x){round(x[["Like_Comp"]],2)}))
rbind(
cbind("q",do.call(cbind,lapply(thislast,function(x){round(x[["q_1"]][1,2],2)}))),
cbind("Npars",do.call(cbind,lapply(thislast,function(x){round(x[["Num_parameters_Est"]],2)}))),
cbind("M",do.call(cbind,lapply(thislast,function(x){round(x[["Mest"]][2],2)}))),
cbind("SigmaR",do.call(cbind,lapply(thislast,function(x){round(x[["Sigmar"]][2],2)}))),
cbind("EffN_Fish",do.call(cbind,lapply(thislast,function(x){round(mean(x[["EffN_Fsh_1"]][,2]),2)}))),
cbind("EffN_Surv",do.call(cbind,lapply(thislast,function(x){round(mean(x[["EffN_Survey_1"]][,2]),2)}))),
tab,
cbind("F2014",do.call(cbind,lapply(thislast,function(x){round((x[["F_fsh_1"]][38,2]),2)}))),
cbind("F2014/F40%",do.call(cbind,lapply(thislast,function(x){round((x[["F_fsh_1"]][38,2])/(x[["F40_est"]]),2)}))),
cbind("B 1977",do.call(cbind,lapply(thislast,function(x){round((x[["TotBiom"]][1,2]),0)}))),
cbind("CV",do.call(cbind,lapply(thislast,function(x){round((x[["TotBiom"]][1,3])/(x[["TotBiom"]][1,2])*100,0)}))),
cbind("B 2014",do.call(cbind,lapply(thislast,function(x){round((x[["TotBiom"]][38,2]),0)}))),
cbind("CV",do.call(cbind,lapply(thislast,function(x){round((x[["TotBiom"]][38,3])/(x[["TotBiom"]][38,2])*100,0)}))),
cbind("2001 YC",do.call(cbind,lapply(thislast,function(x){round((x[["R"]][26,2]),0)}))),
cbind("CV",do.call(cbind,lapply(thislast,function(x){round((x[["R"]][26,3])/(x[["R"]][26,2])*100,0)}))),
cbind("2006 YC",do.call(cbind,lapply(thislast,function(x){round((x[["R"]][31,2]),0)}))),
cbind("CV",do.call(cbind,lapply(thislast,function(x){round((x[["R"]][31,3])/(x[["R"]][31,2])*100,0)})))
)
