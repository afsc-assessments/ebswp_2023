if(domcmc){
  ...
  #mc <- mc[,c(1:11,37)]
  names(mc) <- c("ObjFun","Steepness","lnR0","Fmsyr","SPR_msy","SER_msy","B0","Bmsy","B100","B19","B20","B/mean(B)","B22/B20%","DynB0","q","x","y")
  dim(mc)
  1/mean(1/mc$Fmsyr)
  mc$Chain=1
  mc$Iteration=1:length(mc$Chain)
  p1 <- mc %>% select(Steepness,lnR0,DynB0,B19,Bmsy,Fmsyr) %>% 
  ...
  #ggplot(mc.t,aes(x=Iteration,y=value)) + geom_line() + .THEME + facet_wrap(~Parameter,scales="free")
  p1 <- mc %>% select(B19) %>% ggplot(aes(B19))+ geom_density(fill="lemonchiffon",alpha=.5 ) + theme_few() + 
            xlab("2019 Female spawning biomass") + 
