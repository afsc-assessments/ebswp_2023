...
inseas$dur <-(inseas$duration)/60
inseas <- data.frame(inseas)
names(inseas)
str(inseas)

inseas$date <-   ymd(paste0(inseas$year,"-",inseas$mo,"-",inseas$day))
d <- inseas
# Get areas ==========
d <- d %>% mutate(FMP_area=ifelse(area<540,"BS",ifelse(area>599,"GOA","AI")), sector=ifelse(sector==1,"CP",ifelse(sector==2,"MS","CV")))
# Remove negative ----
d <- filter(d,dur<40,dur>0,!is.na(dur),!is.na(poll_wt))
str(d)

summary(d$dur)
d <- d  %>% mutate(date=ymd(paste(year,mo,day,sep="-")), week=week(date),seas=ifelse(week>=25,"B","A"))
# EBS only
# get report of chinook bycatch by year and strata ======================================

# CPUE PLots all areas
title="Pollock mean nominal CPUE"
# Map mean pos
d %>% filter(FMP_area=="BS",seas=="B") %>% group_by(year,sector)%>% summarise(lat=mean(lat), lon=mean(lon), catch=sum(poll_wt)) %>%
ggplot(aes(x=lon,y=lat,color=sector,label=year)) + geom_text(size=14) + mytheme

# plot catch by sector
d %>% filter(FMP_area=="BS") %>% group_by(year,sector)%>% summarise(catch=sum(poll_wt)) %>%
ggplot(aes(x=year,y=catch,color=sector)) + geom_line(size=2) + mytheme

# plot mean CPUE
cpue <- d %>% filter(gear==2,OTC>0, poll_wt/OTC>800, FMP_area=="BS"|(FMP_area=="GOA"&sector=="CV")) %>% 
        group_by(year,seas,FMP_area,sector) %>% 
        summarise( time=sum(dur,na.rm=T), catch=sum(poll_wt,na.rm=T) ,cpue=catch/time/1000,cpue2=mean(poll_wt/dur/1000),cpue3=median(poll_wt/dur/1000))
ggplot(cpue,aes(x=year,y=cpue,color=seas)) + ggtitle(title) + 
 geom_line(size=1.2) + geom_point(size=3) + mytheme + geom_smooth() +
 facet_grid(. ~ sector ) + facet_grid(FMP_area ~ .) + facet_grid(FMP_area ~ sector) 
#cpue %>% group_by(year,sector) %>% summarise(catch=sum(catch)) %>% ggplot(aes(x=year,y=catch,color=sector)) + geom_line()

d <- inseas
#Pacific cod mean nominal CPUE (in pelagic nets)"
# Get areas ==========
d <- d %>% mutate(FMP_area=ifelse(area<540,"BS",ifelse(area>599,"GOA","AI")), sector=ifelse(sector==1,"CP",ifelse(sector==2,"MS","CV")))
# Remove negative ----
d <- filter(d,dur<40,dur>0,!is.na(dur),!is.na(pcod_wt)) %>% mutate(date=ymd(paste(year,mo,day,sep="-")), week=week(date),seas=ifelse(week>=25,"B","A"))

# plot catch by sector
title="Pacific cod catch in pelagic nets"
d %>% filter(FMP_area=="BS"|FMP_area=="GOA",gear==2) %>% group_by(year,sector,FMP_area)%>% summarise(catch=sum(pcod_wt)) %>%
ggplot(aes(x=year,y=catch,color=sector)) + geom_line(size=2) + mytheme + facet_wrap(~FMP_area) + ggtitle(title)

# plot mean CPUE
title="Pacific cod mean nominal CPUE (in pelagic nets)"
cpue <- d %>% filter(gear==2,OTC>0, FMP_area=="BS"|(FMP_area=="GOA"&sector=="CV")) %>% 
        group_by(year,seas,FMP_area,sector) %>% 
        summarise( time=sum(dur,na.rm=T), catch=sum(pcod_wt,na.rm=T) ,cpue=catch/time/1000,cpue2=mean(pcod_wt/dur/1000),cpue3=median(pcod_wt/dur/1000))
ggplot(cpue,aes(x=year,y=cpue,color=seas)) + ggtitle(title) + 
 geom_line(size=1.2) + geom_point(size=3) + mytheme + 
 facet_grid(. ~ sector ) + facet_grid(FMP_area ~ .) + facet_grid(FMP_area ~ sector) 
#cpue %>% group_by(year,sector) %>% summarise(catch=sum(catch)) %>% ggplot(aes(x=year,y=catch,color=sector)) + geom_line()

 # Chinook per t of pollock
 title="Chinook per t of pollock"
d <- inseas
d <- d %>% mutate(FMP_area=ifelse(area<540,"BS",ifelse(area>599,"GOA","AI")), sector=ifelse(sector==1,"CP",ifelse(sector==2,"MS","CV")))
# Remove negative ----
d <- filter(d,dur<40,dur>0,!is.na(dur),!is.na(poll_wt)) %>% mutate(date=ymd(paste(year,mo,day,sep="-")), week=week(date),seas=ifelse(week>=25,"B","A"))
cpue <- d %>% filter(gear==2,OTC>0, FMP_area=="BS"|(FMP_area=="GOA"&sector=="CV")) %>% 
        group_by(year,seas,FMP_area,sector) %>% 
        summarise( time=sum(dur,na.rm=T), catch=sum(Chin,na.rm=T) ,cpue=catch/time,cpue2=mean(Chin/poll_wt),cpue3=median(Chin/poll_wt))
ggplot(cpue,aes(x=year,y=cpue,color=seas)) + ggtitle(title) + 
 geom_line(size=1.2) + geom_point(size=3) + mytheme +  ylab("Chinook per t of pollock") +
 facet_grid(. ~ sector ) + facet_grid(FMP_area ~ .) + facet_grid(FMP_area ~ sector) 


names(cpue)
#ggplot(cpue,aes(x=year,y=cpue,color=seas,linetype=sector)) + ggtitle(title) + 


ggplot(eff,aes(x=week,y=catch,color=as.factor(year),linetype=as.factor(year))) + ggtitle(title) + geom_line()+geom_point() + .THEME
