...
p_mature *= 0.5;
 ...
 n_pred_grp = n_pred_grp_nonpoll + n_pred_grp_poll;         // the number of predator groups
  N_pred.allocate(1,n_pred_grp,1964,2014,"N_pred");
  ...
  poll_dist.allocate(1,n_pred_ages,1964,2014,1,nstrata_pred,"poll_dist");
  pred_dist_nonpoll.allocate(1,n_pred_grp,1964,2014,1,nstrata_pred,"pred_dist_nonpoll");
  Npred_bystrata_nonpoll.allocate(1964,2014,1,n_pred_grp_nonpoll,1,nstrata_pred);
  area_pred.allocate(1,nstrata_pred,"area_pred");
  nyrs_cons_nonpoll.allocate("nyrs_cons_nonpoll");
  yrs_cons_nonpoll.allocate(1,nyrs_cons_nonpoll,"yrs_cons_nonpoll");
  ...
  sam_oac_cons_nonpoll.allocate(1,n_pred_grp_nonpoll,1,nyrs_cons_nonpoll,"sam_oac_cons_nonpoll");
  ...
  temp_bystrata.allocate(1964,2014,1,nstrata_pred,"temp_bystrata");
  mn_wgt_nonpoll.allocate(1,n_pred_grp_nonpoll,1964,2014,"mn_wgt_nonpoll");
  Y_nonpoll.allocate(1,n_pred_grp_nonpoll);
  Z_nonpoll.allocate(1,n_pred_grp_nonpoll);
  X_nonpoll.allocate(1,n_pred_grp_nonpoll);
  V_nonpoll.allocate(1964,2014,1,n_pred_grp_nonpoll,1,nstrata_pred);
  F_t_nonpoll.allocate(1964,2014,1,n_pred_grp_nonpoll,1,nstrata_pred);
  Cmax_nonpoll.allocate(1964,2014,1,n_pred_grp_nonpoll,1,nstrata_pred);
 ...
 for (j=1;j<=nages;j++) age_vector(j) = double(j);
...
{
  L1.set_initial_value(32.8373996318);
  L2.set_initial_value(53.3070669511);
  log_alpha.set_initial_value(-11.5040644174);
  log_K.set_initial_value(-0.315962350542);
  ...
  log_a_II.set_initial_value(-6.0);
  log_b_II.set_initial_value(3.912);
}
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
 ...
 MN_const = 1e-3; // Multinomial constant
  ...
  mnwt.allocate(age_st,age_end,"mnwt");
    ...
    mnwt.initialize();
  #endif
  coh_eff.allocate(styr_wt-nages_wt-age_st+1,endyr_wt-age_st+3,-15,15,5,"coh_eff");
  yr_eff.allocate(styr_wt,endyr_wt+3,-15,15,4,"yr_eff");
  ...
  resid_temp_int.allocate(do_temp_phase,"resid_temp_int");
  ...
  #endif
  log_a_II.allocate(1,n_pred_grp,1,n_pred_ages,-12,0,do_pred_phase,"log_a_II");
  log_b_II.allocate(1,n_pred_grp,1,n_pred_ages,0,12,do_pred_phase,"log_b_II");
  ...
  #endif
  meanN.allocate(styr,endyr_r,1,n_pred_ages,1,nstrata_pred,"meanN");
  #ifndef NO_AD_INITIALIZE
    meanN.initialize();
  #endif
  meannatage.allocate(styr,endyr,1,nages,"meannatage");
  #ifndef NO_AD_INITIALIZE
    meannatage.initialize();
  #endif
  meannatage_bystrata.allocate(styr,endyr,1,nages,1,nstrata_pred,"meannatage_bystrata");
  #ifndef NO_AD_INITIALIZE
    meannatage_bystrata.initialize();
  #endif
  mean_dens_bystrata.allocate(styr,endyr,1,nages,1,nstrata_pred,"mean_dens_bystrata");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  implied_cpuppa.allocate(1,n_pred_grp,1,nyrs_cons_nonpoll,1,n_pred_ages,1,nstrata_pred,"implied_cpuppa");
  #ifndef NO_AD_INITIALIZE
    implied_cpuppa.initialize();
  #endif
  implied_obs_cons_bystrata.allocate(1,n_pred_grp,1,nyrs_cons_nonpoll,1,n_pred_ages,1,nstrata_pred,"implied_obs_cons_bystrata");
  #ifndef NO_AD_INITIALIZE
    implied_obs_cons_bystrata.initialize();
  #endif
  implied_prop_Cmax.allocate(1,n_pred_grp,1,nyrs_cons_nonpoll,1,n_pred_ages,1,nstrata_pred,"implied_prop_Cmax");
  #ifndef NO_AD_INITIALIZE
...
void model_parameters::preliminary_calculations(void)
{
  ...
  for (i=1;i<=n_eit_r;i++) lseb_eit(i) = (std_ob_eit(i)/ob_eit(i));
...
{
  dvector temp1("{50,50,100,200,500,5000}");
...
void model_parameters::userfunction(void)
{
  ...
  GetNumbersAtAge();
  Get_Catch_at_Age();
  if(active(log_a_II))                // added by Paul
    Get_Cons_at_Age();
...
void model_parameters::report(const dvector& gradients)
{
  ...
  */ 
  ...
  ofstream est_cons_out("est_cons_out.dat");
  est_cons_out << "predator year  obs_cons_nonpoll pred_cons"<<endl;
  for (j=1;j<=2;j++){
  for (i=1;i<=nyrs_cons_nonpoll;i++) {
     iyr = yrs_cons_nonpoll(i);
     est_cons_out <<j<<" "<<iyr<<" "<<obs_cons_nonpoll(j,i)<<" "<<pred_cons(j,iyr) <<endl;
  }
  }
  report <<"consumption ssq are "<< endl;
  report << ssq_cons << endl;
  report <<"the resid M like is  "<< endl;
  report << sum(resid_M_like) << endl;
  ofstream meanM_out("meanM_out.dat");
  ...
  for (j=1;j<=n_pred_grp;j++) {
     report << "the observed age comps for predator "<<j << endl;
    for (i=1;i<=nyrs_cons_nonpoll;i++){
         iyr = yrs_cons_nonpoll(i);     
         report <<iyr<<" "<<oac_cons_nonpoll(j,i)<<endl;
       }
    report << "the estimated age comps for predator "<<j << endl;     
    for (i=1;i<=nyrs_cons_nonpoll;i++){
         iyr = yrs_cons_nonpoll(i);          
         report <<iyr<<" "<<eac_cons(j,iyr)<<endl;
       }
   }
  report << " the predator age comps likelihood is "<<endl;
  report <<age_like_cons << endl;
  ofstream cpuppa_out("cpuppa_out.dat");
  cpuppa_out <<" predator year  age strata implied_cpuppa meanN implied_prop_Cmax "<<endl; 
   for (j=1;j<=n_pred_grp;j++) {
     for (i=1;i<=n_pred_ages;i++) {
       for (k=1;k<=nyrs_cons_nonpoll;k++){
        for (z=1;z<=nstrata_pred;z++){
         cpuppa_out << j <<" "<<k<<" "<<i<<" "<<z<<" "<<implied_cpuppa(j,k,i,z)<<" "<<mean_dens_bystrata(yrs_cons_nonpoll(k),i,z)<<" "<<implied_prop_Cmax(j,k,i,z) <<endl;
       }
       }
      }
     }
  //  for (j=1;j<=n_pred_grp;j++) {
  //   report << "the observed CPUP for predator "<<j << endl;
  //  for (i=1;i<=nyrs_cons_nonpoll;i++){
  //       iyr = yrs_cons_nonpoll(i);     
  //       report <<iyr<<" "<<obs_cpup_nonpoll(j,i)<<endl;
  //     }
  //  report << "the predicted CPUP for predator "<<j << endl;     
  //  for (i=1;i<=nyrs_cons_nonpoll;i++){
  //       iyr = yrs_cons_nonpoll(i);          
  //       report <<iyr<<" "<<pred_cpup(j,iyr)<<endl;
  //     }
  // }
  //report << "the ssq_cpup are: "<<endl;
  //report << ssq_cpup << endl;
  report << " the mean density across strata are " << endl;
  report << "Year  age1 age2 age3 "<< endl;
  for (i=styr;i<=endyr_r;i++)
     report <<i<<" "<<mean_dens(i) <<endl;
  report << " the function response parameters are "<< endl;
    for (j=1;j<=n_pred_grp;j++) {
       report << "predator  "<<j <<" a: "<<mfexp(log_a_II(j)) << endl;
       report << "predator  "<<j <<" b: "<<mfexp(log_b_II(j)) << endl;
    }
   report <<" the meannatage is "  << endl;
   for (i=styr;i<=endyr_r;i++)
     report <<i<<" "<<meannatage(i) <<endl;
}
  ...
  for (k=1;k<=n_pred_ages;k++){           // loop over ages of pollock that are preyed upon 
   ...
   natage_strat_dens(i,k) = elem_div(natage_strat(i,k),area_pred);  // the density of age k pollock in each area
     for (j=1;j<=nstrata_pred;j++){            // loop over strata, and get the mean abundance accounting for all mortality
       M_pred(i,k,j) = get_Mpred(natage_strat_dens(i,k,j),F(i,pred_ages(k)),resid_M(k),
                      column(a_II,k),column(b_II,k),column(Npred_bystrata_nonpoll(i),j),column(mn_wgt_nonpoll,i)/(wt_bts(yr_ind,k)*1000),column(Cmax_nonpoll(i),j));
...
dvar_vector model_parameters::get_Mpred(const dvariable& tmp_abun_bg,const dvariable& tmp_F,const dvariable& tmp_M,
                const dvar_vector& a,const dvar_vector& b,const dvector& Npred, const dvector& wt_ratio, const dvector& tmp_Cmax)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
void model_parameters::Get_Catch_at_Age(void)
{
  ...
  // changed by paul to use the meannatage from the predation mortalities
  //catage = elem_prod( elem_prod(natage , F ) , elem_div( (1. - S) , Z ));  (Jims original code)
  catage = elem_prod(meannatage,F);                         // chaged by Paul 
  for (i=styr; i<=endyr_r; i++)
...
dvariable model_parameters::get_yield(dvariable& Ftmp, dvariable& Stmp,dvariable& Rtmp,dvariable& Btmp)
{
  ...
  RETURN_ARRAYS_INCREMENT();
  // Note that two wt vectors are used: 1 for yield, the other for biomass.
  // edited by Paul to get the yield with spatial predation (based on survival in endyr_r)  
  dvariable yield;
  dvariable phi;
  phi.initialize();
  dvariable Req;
  ...
  dvar_vector survtmp = mfexp(-Ztmp);
  Ntmp(1) = 1.;
  //dvar_matrix Mvec(1,n_pred_ages,1,nstrata_pred);
  //dvar_matrix Zvec(1,n_pred_ages,1,nstrata_pred);
  //dvar_matrix Svec(1,n_pred_ages,1,nstrata_pred);
  //if(do_pred==1){
  //   for (j=1;j<=n_pred_ages;j++){
  //      Mvec(j) = Z_pred(endyr_r-1,j) - F(endyr_r-1,j);
  //      Zvec(j) = Mvec(j) + Fatmp(j);
  //      Svec(j) = mfexp(-Zvec(j));
  //      Ntmp(j+1) = (poll_dist(j,endyr_r-1)*Ntmp(j))*Svec(j);     // the natage after predation
  //   }  
  //   Ntmp(n_pred_ages+2,nages) = ++elem_prod(Ntmp(n_pred_ages+1,nages-1), survtmp(n_pred_ages+1,nages-1));
  // }  // do_pred loop
  // else {
  for ( j=1 ; j < nages; j++ )
    Ntmp(j+1)  =   Ntmp(j) * survtmp(j); // Begin numbers in the next year/age class
  ...
  //if(do_pred==1){
  // for (j=1;j<=n_pred_ages;j++)
  //      Ctmp(j)     = sum(elem_div(elem_prod(Ntmp(j)*poll_dist(j,endyr_r-1)*Fatmp(j),(1. - Svec(j))),Zvec(j)));
  // for (j=n_pred_ages+1;j<=nages;j++) 
  //      Ctmp(j)     = Ntmp(j) * Fatmp(j) * (1. - survtmp(j)) / Ztmp(j); 
  //   }  // do pred loop
  //else {
  for ( j=1 ; j <= nages; j++ )
...
dvariable model_parameters::SRecruit_ps(const dvariable& Stmp, double sst)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
void model_parameters::Get_Bzero(void)
{
  ...
  switch (SrType)
  {
    case 1:
      ...
      //alpha = (log(steepness/0.2))/0.8; //  ****** added by paul for Ricker model steepness-alpha relation
      break;
    case 2:
...
void model_parameters::Recruitment_Likelihood(void)
{
  ...
  else
  {
    ...
    for (i=styr_est;i<=endyr_est;i++)
    {
       srmod_rec(i) = SRecruit_ps(SSB(i-1),SST(i-1)); // 1 year lag w/ SSB and sst
    }
    SR_resids = log(pred_rec(styr_est,endyr_est)+1.e-8) - log(srmod_rec + 1.e-8)  ;
    if (active(resid_temp_int)){    // added by Paul
        SR_resids_temp = ++(resid_temp_int + resid_temp_x1*SST + resid_temp_x2*elem_prod(SST,SST));  //***** added by Paul ******
...
void model_parameters::Evaluate_Objective_Function(void)
{
  ...
  if (active(resid_temp_int)) fff += SR_resids_like; //****** added by Paul ****** // 
    ...
    if(active(log_a_II)){
     ...
     //ssq_cpup.initialize();
      for (i=1;i<=nyrs_cons_nonpoll;i++){
         iyr = yrs_cons_nonpoll(i); 
         for (j=1;j<=n_pred_grp;j++){
           ssq_cons(j) += square(log(obs_cons_nonpoll(j,i)+1e-4)-log(pred_cons(j,iyr)+1e-4));
           fff += ssq_cons(j);           
...
dvar_matrix model_parameters::compute_selectivity(int stsel,const dvariable& slp,const dvariable& a50)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity(int stsel,const dvariable& slp,const dvariable& a50,const dvar_vector& a50_dev)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity(int stsel,const dvariable& slp,const dvariable& a50,const dvar_vector& se,const dvar_vector& ae)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity(int stsel,const dvar_vector& slp,const dvar_vector& a50)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity1(int stsel,const dvariable& dif,const dvariable& a50,const dvariable& trm)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity1(int stsel,const dvariable& dif,const dvariable& a50,const dvariable& trm,const dvar_matrix& devs)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity2(int stsel,const dvar_vector& slp,const dvar_vector& a50,const dvar_matrix& devs)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity3(int stsel,const dvar_vector& dif,const dvar_vector& a50)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity3(int stsel,const dvar_vector& dif,const dvar_vector& a50,const dvar_matrix& devs)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity(int stsel,const dvar_vector& slp,const dvar_vector& a50,const dvar_matrix& devs)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_fsh_selectivity(int nsel,int stsel,dvariable& avgsel,const dvar_vector& coffs,const dvar_matrix& sel_devs,int  gn)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity_eit(int nsel,int stsel,dvariable& avgsel,const dvar_vector& coffs)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity_eit(int nsel,int stsel,dvariable& avgsel,const dvar_vector& coffs,const dvar_matrix& sel_devs)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity(int nsel,int stsel,dvariable& avgsel,const dvar_vector& coffs,const dvar_matrix& sel_devs,int  gn)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity(int nsel,int stsel,dvariable& avgsel,const dvar_vector& coffs)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
void model_parameters::Write_R(void)
{
  ...
  for (i=styr;i<=endyr_r;i++) 
    R_report <<i<<" "<< 
    elem_prod( elem_prod(
        elem_prod(natage(i),pow(S(i),yrfrac)), p_mature),
            wt_ssb(i))
            /SSB(i) <<endl;
  R_report(wt_ssb);
  R_report(wt_cur);
...
void model_parameters::Fit_resid_M(void)
{
  ...
  ofstream& eval= *pad_eval;
  random_number_generator& rng= *pad_rng;
  //  fit the residual M so such that the total M for an age (residual plus predation mortality) is close to Jimi's fixed values
  int k; 
  for (k=1;k<=n_pred_ages;k++){
      resid_M_like(k) = 50.0*norm2( (M_pred_avg(k) + mfexp(log_resid_M(k))) - natmort(k));
  }   
