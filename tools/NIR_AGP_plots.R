...
flextable(tab_fit(modlst,mod_scen=c(1,6:7)))
M<- modlst[6:7]
names(M[1])
               M[[1]]$age_like[1]
               M[[2]]$age_like[1]
               ...
               sum(M[[1]]$age_like_yr[1,])- M[[1]]$age_like_offset[1]
               sum(M[[2]]$age_like_yr[1,],na.rm=TRUE)- M[[2]]$age_like_offset[1]
               
               sum(M[[1]]$age_like_yr[2,],na.rm=T)- M[[1]]$age_like_offset[2]
               sum(M[[2]]$age_like_yr[2,],na.rm=T)- M[[2]]$age_like_offset[2]
               
df<-data.frame(year=1964:2021,
               AGP=M[[1]]$age_like_yr[1,],
               NIR=M[[2]]$age_like_yr[1,]
               ) |> 
  pivot_longer(cols=2:3,names_to="source", values_to="NLL")
df |> filter(year>2010) |> group_by(source) |> mutate(cum=cumsum(NLL)) |> 
ggplot(aes(x=year,y=NLL,color=source)) + geom_line()+ggthemes::theme_few()
M[[2]]$yrs_
if (doplots) {
  do_data_plots <- FALSE
...
#---Recruit
  ...
  p1 <- plot_recruitment(modlst[c(1,6)],xlim=c(2000.5,2022.5))+ylab("");p1
  p1 <- plot_recruitment(modlst[c(7,6)],xlim=c(2000.5,2022.5))+ylab("");p1
  p1 <- plot_recruitment(modlst[c(7,6)],xlim=c(2000.5,2022.5))+ylab("");p1
  p1 <- plot_recruitment(modlst[c(4:5)],xlim=c(2000.5,2022.5))+ylab("");p1
  p1 <- plot_recruitment(modlst[c(2,3)],xlim=c(2000.5,2022.5))+ylab("");p1
#---SSB-------------------------
  p1 <- plot_ssb(modlst[c(1,6)],xlim=c(2004.5,2022.5),breaks=seq(2008,2024,by=2),alpha=.2); p1
  p1 <- plot_ssb(modlst[c(6,7)],xlim=c(2004.5,2022.5),breaks=seq(2008,2024,by=2),alpha=.2); p1
  p1 <- plot_ssb(modlst[c(2,4)],xlim=c(2004.5,2022.5),breaks=seq(2008,2024,by=2),alpha=.2); p1
  ggsave("figs/mod_bridge.pdf",plot=p1,width=8,height=5.0,units="in")
...
#---BTS Fit---------------------
  ...
  p1 <- plot_bts(modlst[c(1,6)],xlim=c(1982.5,2022.5),ylim=c(0,15000)) ;p1
  p1 <- plot_bts(modlst[c(6,7)],xlim=c(1982.5,2022.5),ylim=c(0,15000)) ;p1
  p1 <- plot_ats(modlst[c(2,3)],xlim=c(1982.5,2022.5),ylim=c(0,15000)) ;p1
...
#---Selectivity-----------------
  ...
  ggsave("figs/mod_fsh_sel.pdf",plot=p1,width=4,height=9,units="in")
 ...
 # p1 <- plot_mnage(modlst[c(2,3)]) 
  ...
  p1 <- plot_mnage(modlst[2:3]) ;p1 # Note used model 1 for figure...because of kludge for age compos
  ggsave("figs/mod_mean_age.pdf",plot=p1,width=5.8,height=8,units="in")
  p1 <- plot_bts(modlst[1:3]) ; p1
...
modlst
  p1 <- plot_srr(modlst[c(4,5)],alpha=.2,xlim=c(0,5200),ylim=c(0,80000),sizeout=1,sizein=4,yrsin=c(1977,1979:2020));p1
#  p1 <- plot_srr(modlst[c(1,2)],alpha=.2,xlim=c(0,5200),ylim=c(0,100000));p1
  ...
  p1 <- plot_srr(modlst[c(1,6)],alpha=.2,xlim=c(0,4500),ylim=c(0,85000),
  ...
  pdf("../doc/figs/nir_agp_agefits.pdf",width=6,height=8)
  plot_agefit(modlst[[6]],case_label=paste0("AGP ",af_title),gear="bts",type="survey",styr=1994,ageplus=10)
  plot_agefit(modlst[[7]],case_label=paste0("NIR ",af_title),gear="bts",type="survey",styr=1994,ageplus=10)
  
  plot_agefit(modlst[[6]],case_label=paste0("AGP ",af_title),gear="fsh",type="fishery",styr=1994,ageplus=10)
  plot_agefit(modlst[[7]],case_label=paste0("NIR ",af_title),gear="fsh",type="fishery",styr=1994,ageplus=10)
  
  plot_agefit(modlst[[6]],case_label=paste("AGP",af_title),gear="ats",type="survey",styr=2001)
  plot_agefit(modlst[[7]],case_label=paste("NIR",af_title),gear="ats",type="survey",styr=2001)
...
hlr <- M$regime[2]
  ...
  ggsave("figs/mod_regimes.pdf",plot=p3,width=9.2,height=5.0,units="in")
...
#---Catch grid and future effort consequences---------------------------------------------------
  ...
  p1 <- ggplot(df%>%filter(Area=="Both"),aes(x=Year,y=Biomass)) + geom_point(size=3,color="red",position=position_dodge(width=0.5)) + 
      geom_errorbar(aes(ymin=ll,ymax=ul),width=.5,position=position_dodge(width=0.5)) + theme_few(base_size=19) + 
      scale_x_continuous(breaks=seq(1982,2022,by=4))  + ylim(c(0,15)) +
      ylab("Biomass (millions of t)"); p1
  ggsave("figs/bts_biom.png",plot=p1,width=9.4,height=7,units="in")
  ggsave("figs/bts_biom.pdf",plot=p1,width=9.4,height=7,units="in")
...
p1 <- df %>% filter(!is.na(strata)) %>% mutate(strata=ifelse(strata==1,"A-season",ifelse(strata==2,"NW B-season","SE B-season"))) %>% group_by(Year,strata) %>% summarize(Catch=sum(Catch)) %>%
      ...
      p1
ggsave("figs/catch.pdf",plot=p1,width=7.5,height=4.5,units="in")
# flextable(tsum)
    #---Roe data------------------
  ...
  ggsave("figs/vastage_vs_db.pdf",plot=p1,width=7,height=9,units="in")
#---Catch data table-------------------------------
