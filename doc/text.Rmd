...
spawning earlier or later (refs to come).

Eggs and larvae of EBS pollock are planktonic for a period of about 90 days
and appear to be sensitive to environmental condations. These conditions
...
million t in 2009 to nearly 1.5 million t during 2003-2006 (Table \ref{tab:catch}).  
During a 10-year period, catches by foreign vessels operating in the “Donut Hole”
...
foreign vessels since the late 1970s. 
...
measures are shown in Table \ref{tab:abc_tac}. 
...
season and broad area has varied over time with recent B-season catches
occuring in the southeast portion of the shelf (east of 170$^o$W longitude;
...
The catch estimates by sex
for the seasons indicate that over time, the number of males and females has
been fairly equal (Fig. \ref{fig:catch_sex}). The 2017 A-season fishery
spatial pattern had relatively high concentrations of fishing on the shelf
north of Unimak Island, especially compared to the pattern observed in 2015
when most fishing activity occurred farther north (Fig.
\ref{fig:catch_distn_a}). The catch rate fleet-wide continued from the good
conditions observed in 2016 summer-fall period (Fig. \ref{fig:aseas_cpue}).
Also of note for this year was that, due to  a regulatory change, up to 45\%
of the TAC could be taken in the A-season. This conservation measure was made
to allow greater flexibility to avoid Chinook salmon in the B-season. To date,
it appears that the pollock fleet as a whole took advantage of  this added
flexibility (Fig. \ref{fig:prop_a_season}).

The 2017 summer and fall (B-season) fishing was had similar pattern that was
...
but was still quite good compared to other recent years (Fig.
\ref{fig:bseas_cpue}). Since 1979 the catch of EBS pollock has averaged 1.19
million t with the lowest catches occurring in 2009 and 2010 when the limits
were set to 0.81 million t due to stock declines (Table \ref{tab:catch_disc})
Pollock retained and discarded catch (based on NMFS observer estimates) in the
Eastern Bering Sea and Aleutian Islands for 1991-2016 are shown in (Table
...
to high of 51% in 2016 (Table \ref{tab:catch_sca}). This high value was due to B-season
...
capacity in the near future, though fillets are still a small portion of their
...
\begin{tabular}{p{1.5in}p{1.8in}p{3.2in}}
\hline
 Source & Type & Years \\
\hline
...
were:  i) January–June (all areas, but mainly east of 170$^o$W); ii) INPFC area
51 (east of 170$^o$W) from July–December; and iii) INPFC area 52 (west of 170$^o$W)
...
class) were present. In 2012 - 2016 the 2008 year class been prominent in the
...
effort for age determinations and lengths is shown in Tables \ref{tab:fsh_meas} and \ref{tab:fsh_n}.
...
new high in 2016 and dropped to more average values this year (Fig. \ref{fig:bts_temp}).
...
and were abundant in locales cooler than 2 degrees C bottom temperatures
...
al. (2006, 2011). Kotwicki et al. (2013) also found that that the catchability
...
an contains the values used for the index which
...
The AT surveys are conducted biennially (most recently in 2016) and are designed to estimate the off-
...
presented in (Table \ref{tab:ats_n}). Estimated midwater pollock biomass for
the shelf was above 4 million tons in the early years of the time series
(Table \ref{tab:ats_biom}). It dipped below 2 million t in 1991, and then
...
lower midwater biomass estimates between 2006 and 2012 (the recent ‘cold’
period; Honkalehto and McCarthy 2015). The midwater pollock biomass estimate
from the 2016 AT survey of 4.06 million is above the average (2.76 million t).
Previously relative estimation errors for the total biomass were derived from
a one-dimensional (1D) geostatistical method (Petitgas 1993, Walline 2007,
Williamson and Traynor 1996). This method accounts for observed spatial
structure for sampling along transects. As in previous assessments, the other
sources of error (e.g., target strength, trawl sampling) were accounted for by
inflating the annual error estimates to have an overall average CV of 25% for
application within the assessment model (based on judgement relative to other
indices).

The 2016 summer AT survey age compositions were updated using age-length keys
from the actual survey stations (in 2016 the keys from were derived mainly
from the BTS collections; Fig. \ref{fig:at_age}); Table \ref{tab:ats_age}). 
Additionally, historical data were updated to account for the layer of pollock
detected between 0.5 and 3m from the bottom (previous estimates had use pollock 
estimates between the surface down to 3m from the bottom only). This affected the 
age compositions as well although the changes are relatively minor (Fig. \ref{fig:at_age_new}).
Spatially, the 2016 survey observed relatively few age 1 pollock whereas age 3 (the 2013
year class) was the most abundant age group followed by four year olds.
The portion of shelf-wide biomass estimated to be east of
170$^o$ W was 37%, compared to an average of 24% since 1994 (Table \ref{tab:ats_biom}).  Also,
the distribution of pollock biomass within the SCA was similar to that found
in 2014 at 13% compared to the 2007-2012 average of 7% (and 1994-2016 average
of 10%).
...
among key prey and predator species and for pollock, it and can be used to
...
compositions for the new series, the plan is to incorporate and present these
...
the patterns seen in the  BTS survey data as they data show increased
...
interactions with age-specific selectivity require consideration. For the BTS index, 
selectivity-at-age estimted with 
age and year specific deviations in the average selectivity-at-age. 

Time-varying survey selectivity is estimated to account for the
changes in availability of pollock to the survey gear and is constrained by
pre-specified variance terms. For the AT
survey, which originally began in 1979 (the current series including data down to 
0.5m from bottom begins in 1994), optional parameters to allow for age and time-varying patterns
exist but for this assessment and other recent assessments, ATS selectivity is constant over time. 
Overall, five catchability coefficients were estimated:
one each for the early fishery catch-per-unit effort (CPUE) data (from Low and
Ikeda, 1980), the early bottom trawl survey data (where only 6 strata were
surveyed), the main bottom trawl survey data (including all strata surveyed),
the AT survey data, and the AVO data. An uninformative prior distribution as used for all
of the indices. The selectivity parameters for the 2 main indices total 135
...
*	Fishery and survey proportions-at-age estimates (multinomial 
 	with effective sample sizes presented later in (Table \ref{tab:`r tablab[23]`}).      
...
the estimation the of the stock-recruitment relationship by including the
...
degree the extent to which the fishery is focused on particularly prominent
year-classes (Fig. \ref{fig:mod_fsh_sel}). The model fits the fishery age-composition data
quite well under this form of selectivity (Fig. \ref{fig:mod_fsh_age}). The fit to the early
Japanese fishery CPUE data (Low and Ikeda 1980) is consistent with the
population trends for this period (Fig. \ref{fig:mod_cpue_fit}). The fit to the fishery-
independent index from the 2006-2017 AVO data shows 
a relatively stable trend in recent years (Fig. \ref{fig:mod_avo_fit}).
...
\ref{tab:tier3}). This compares with the `r round(100*curssb/b100,0)`\% 
...
mass. The uncertainty  the average weights-at-age projected for the fishery and
...
```{r abctab13}
df <- data.table(Tier=c("1a","1a","3a","3a"), Year=c(nextyr,nextyr+1,nextyr,nextyr+1),MaxABC=c(maxabc1,maxabc2,abc1,abc2),OFL=c(ofl1,ofl2,Tier3_OFL1,Tier3_OFL2))
tab <- xtable(df, digits = c(0,0,0,0,0),big.mark=",")
print(tab,include.rownames=F,big.mark=",")
```
...
`r thisyr`, and it is not expected to be approaching an overfished condition based on
Scenario 7 (the mean spawning biomass in `r thisyr` is above the $B_{35\%}$ level;
((Table \ref{tab:tier3_SSB}). Tier 1 calculations for ABC and OFL values in
`r thisyr` and `r nextyr` (assuming catch is 1,350,000 t in `r nextyr` are given in (Table
\ref{tab:tier1}). Based on this, the EBS pollock stock is not being
subjected to overfishing, is not overfished, and not approaching a condition
of being overfished.
...
2016 should help reduce Chinook salmon bycatch. Despite the high catch rates
observed in 2017 (particularly in the A-season), the Chinook salmon bycatch is the 
highest on recent record (Table \ref{tab:pscbycatch}). Also, the catch of (mainly)
chum salmon in the non-Chinook bycatch estimates was also very high during the 2017
B-season fishery.
...
A separate section presented again this year updates a multispecies model with more
