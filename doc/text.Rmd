...
#### Changes in the data
   ...
   bottom trawl survey and used as an added index of pollock biomass in mid-
   water. 2.  Observer data for catch-at-age and average weight-at-age from
...
$F_{OFL}$                             &	0.621	      &	0.621	      &		`r (M$arithmeanF)`	   &	`r (M$arithmeanF)`	\\
$maxF_{ABC}$                          & 0.466	      &	0.466	      &	  `r (M$harmeanF)`	     &	`r (M$harmeanF)`  	\\
$F_{ABC}$                             &	0.336       &	0.336       &		`r (M$fabc)`           &	`r (M$fabc)`        \\
...
individuals stocks as well as the overall ecosystem. Some species, such as
...
Nunivak Island to the Russian convention line and into Norton Sound), as shown
below and evaluated in the appendix, suggests that there are concentrations of
pollock present which contrasts with the 2010 survey when relatively few
pollock were present. The pattern of temperatures in the region likely affect
the pollock distribution in ways that likely vary over time.  However, there is
evidence of a relationship between mean bottom temperatures in
the US zone on the EBS shelf and subsequent biomass estimates in the  Navarin
basin (the Russian area adjacent to the Convention Line; e.g., Stepanenko and Gritsay 2018, 
Ianelli et al. 2015). Some genetic samples were taken from pollock and collections
continue. Pending funding availability, analysis of these samples could 
help ascertain the extent that these fish are related to those observed in the
normal EBS shelf survey area.   Genetic samples taken from the 2017 summer
bottom trawl survey from the Northern Bering Sea can be compared with samples from the
standard Bering Sea Unimak, Pribilof, Bogoslof, and Zhemchug. This planned study should
help improve stock structure evaluation (last done in Ianelli et al. 2015).
...
seasons (Fig. \ref{fig:aseas_cpue}). Beginning in 2017,
due to  a regulatory change, up to 45\% of the TAC could be taken in the
A-season. This conservation measure was made to allow greater flexibility to
avoid Chinook salmon in the B-season. To date, it appears that the pollock
fleet as a whole took advantage of  this added flexibility (Fig.
\ref{fig:prop_a_season}).
...
observer estimates) in the Eastern Bering Sea and Aleutian Islands for 1991-`r
thisyr` are shown in (Table \ref{tab:catch_disc}). Since 1991, estimates of
...
implementation of the Council’s Improved Retention /Improved Utilization
...
The majority (about 56\%) of Chinook salmon caught as bycatch in
...
Aleutian Island (BSAI) region. In 2017 they accounted for 70\% of the BSAI's FMP
groundfish harvest and 88\% of the total pollock harvest in Alaska. Retained
catch of pollock increased 0.5\% to 1.36 million t in 2017. BSAI pollock
first-wholesale value was \$1.355 billion 2017, which was roughly equal to the value in
in 2016 and above the 2005-2007 average of \$1.25 billion. The higher revenue in
...
though this varies across products types. The marginal increases in the average 
first-wholesale price of pollock products in 2016 and 2017 are largely due to increases
the price of surimi products.
...
uncertiantly  could negatively affect the market.
...
the supply of raw surimi material continues to be constrained in Japan. A
trend which is expected to continue  through 2018. Increasing Atka mackerel
prices (another source of raw material  for surimi) could also increase demand
for pollock based surimi.
...
The Yen to U.S. Dollar exchange rate, which can influence  prices, has
remained relatively stable. The average roe price in the BSAI was up 2.2\%  in
...
2010 but declined again in 2012-2013. However, in 2014-2016 bottom
temperatures increased and reached a new high in 2016. In 2018 bottom temperatures were
nearly as warm (after 2017 which was just slightly above average; Fig. \ref{fig:bts_temp}) 
However, 2018 marked an unusual year temperature-wise as there was a complete lack of a "cold pool"
\footnote{The traditional area-swept design-based index is reported in some tables along with the density-dependent 
...
average (5%) at 4%   and 3% (Table \ref{tab:bts_biom}). In some years (e.g.,
...
The 2018 biomass estimate (design-based, area swept) was 3.11 million t, below
...
this year (but was limited to 40 stations). Given that the pollock abundance
...
(VAST) model of Thorson et al. (2015, 2016) together with the density-
dependent corrected CPUE values from each station (including stations where
pollock were absent). Please refer to the appendix for further details on the
implementation. The appendix also shows results that indicate the VAST model
diagnostics are reasonable and provide consistent interpretations relative to
the observations. Notably, results indicate increased uncertainty in years and
areas when stations were missing.  Application of this index within the stock
assessment model requires accounting for the temporal covariation. Since this
has been part of the assessment for the time series of biomass used  in past
years, including the covariance specification was simple to implement and
required no changes to the assessment model code.
...
The AT surveys are conducted biennially and are
...
and 2012 (the recent ‘cold’ period; Honkalehto and McCarthy 2015). 

Due to an unforeseen ship failure, the final 3 transects of the 2018 AT survey
could not be completed (red transects in Fig. \ref{fig:at_unsampled}).  To
estimate the fraction of pollock in this area, we used the opportunistically
collected acoustic data collected by NOAA's  bottom trawl survey vessels (see
Honkalehto et al., 2011 for details). The trawl data available from adjacent
areas surveyed by the RV *Oscar Dyson* during the AT survey were used together
with the acoustic backscatter from the bottom trawl survey boats in the
unsampled area to estimate pollock abundance from that area.  The small area
at the "shelf break" (100-200 m depth) normally covered by the AT survey in
this region was omitted since this area was not covered by the bottom trawl
survey vessels.

The "unsampled area" was from 10 nmi from the Dyson’s last transect out to the
200 m depth contour.  This area (the filled polygon in Fig.
\ref{fig:at_unsampled}) constituted a survey area of 6,016 nmi^2^, which is a
relatively small fraction of the survey area sampled in 2018 (92,283 nmi^2^).
In other words, about 6.1\% of the normal survey area was missed. The goal of
this exercise was to estimate the abundance of pollock in this unsampled area.
The 38 kHz acoustic data from the two chartered bottom trawl vessels F/V
*Vesteraalen* and F/V *Alaska Knight* inside the unsampled area were
identified (Fig. \ref{fig:at_unsampled}).  The vessels were equipped with
Simrad ES60 echosounders and ES38B 38 kHz transducers.  Data collected during
daytime at speeds >4 knots were processed in Echoview equivalent methods as
the Dyson survey data. This resulted in 435 nmi of trackline in the unsampled
area on July 27 to July 29, 2018.

Acoustic data were post-processed in a manner analogous to the AT survey. The
average of 2 sphere  The average of 2 sphere calibrations conducted before and
after the 2018 bottom trawl survey was applied in post-processing, and regions
consistent with backscatter from walleye pollock were identified.   The
repeating ‘triangle wave’ error present in ES60 data was removed from these
data (Honkalehto et al, 2011).  As with the Dyson survey data, 38 kHz
backscatter consistent with pollock was exported every 0.5 nmi along the
vessel track from 16m above the sea surface to 0.5 m above the sounder-
detected bottom. Abundance was estimated by averaging the available
backscatter observations in the unsampled area, estimating the density of
pollock (i.e., fish nmi^2^), and extrapolating this point estimate of density
over the unsampled area.  Abundance (N, total number of pollock in the
unsampled area) was computed as follows

\begin{align}
   N_{}     &= \frac{\bar{S}_{A}}{4\pi\hat{\sigma}_{bs,PK}}PB_{pk}A 
\end{align}

where $\bar{S}_A$ is the mean nautical area backscattering coefficient in the
unsampled area (Maclennan et al., 2002) in the attributed to pollock unsampled
area, and $PB_{pk}$ is the proportion of this backscatter estimated to be from
pollock. $\bar{S}_A$ was estimated as the average over all 0.5 nmi segments in
the unsampled area, $\hat{\sigma}_{bs,PK}$ is the mean backscattering cross
section of pollock, and $PB_{pk}$ the proportion of $\bar{S}_A$  attributable
to pollock (see De Robertis et al. 2017).

As in the acoustic trawl survey, different methods are used to estimate
abundance <3m from the seafloor and >3m from the seafloor, as the demersal
hauls conducted on the bottom survey trawls were more appropriate descriptors
of the more diverse demersal fish community than the midwater hauls conducted
on the acoustic-trawl survey. For 0.5 to 3m from the sounder-detected bottom,
the method of Lauffenberger et al, 2017 was applied, but with the acoustic
data from the bottom trawl vessels substituted for that from the Dyson to
estimate $\bar{S}_A$. In this method, the bottom trawl hauls conducted during
the bottom trawl survey and a statistical model are used to estimate $PB_{pk}$
and $\hat{\sigma}_{bs,PK}$. The size distribution for the pelagic component in
the unsampled area was based on the adjacent AT survey two transects (i.e.,
transects 24, 25, Fig. \ref{fig:at_unsampled}). This size distribution was
based on 10 midwater hauls with an Aleutian Wing Trawl conducted from 15 to 20
August. In addition, $PB_{pk}$ was estimated as the average of those 10 hauls
($PB_{pk}$ = 0.994).  The abundance at length and age was computed with the
same methods as the AT survey.

The estimate for the unsampled area (0.5m to 16m from surface)  was 616
million pollock with a biomass of 178,194 t. When expressed as the proportion
of the total in the standard AT survey area  (i.e. the traditional survey area
excluding the northern extension),  the estimated biomass in the unsampled
area was 7.1\% of that in the entire survey area (11.1\% in terms of numbers).
This compares to the proportion of the survey area that was unsampled (6.1\%
of the standard AT survey area) and indicates that this area was slightly above 
average densities seen in the standard survey area. A spatial comparison with the
BTS data is shown in Fig. \ref{fig:ats_bts}.

Thus, the combined (including unsampled area estimates) midwater pollock
biomass  estimate was 2.50 million t and is below average (2.82 million t;
Table \ref{tab:ats_biom}). Relative estimation errors for the total biomass
were derived from a one-dimensional (1D) geostatistical method (Petitgas 1993,
Walline 2007, Williamson and Traynor 1996) and account for observed spatial
...
application within the assessment model (based on judgment relative to other
indices). The portion of shelf-wide biomass (from surface to 3m off-bottom)
estimated to be east of 170$^\circ$W was 37%, compared to an average of 24%
since 1994 (Table \ref{tab:ats_biom}).  Also, the distribution of pollock
biomass within the SCA was similar to that found in 2014 at 13% compared to
the 2007-2012 average of 7% (and 1994-2016 average of 10%).

The 2018 EBS acoustic-trawl survey estimates of population numbers at age were
developed based on a mix of samples from the survey and from the bottom trawl
survey (primarily the latter). These samples were used to develop age-length
keys  to apply to the length composition of the acoustic pollock backscatter
observed  from the AT survey (Fig. \ref{fig:at_age}). As adopted last year,
the pollock backscatter from the bottom layer between 0.5 and 3m depth were
included. Interestingly, the 2018 data supports the observation in 2016 that
the 2013 year class was relatively abundant (along with the 2012 year class;
...
analyses include:
  ...
  - The `r thisyr-1` fishery age composition data were added and catch biomass through to
    the current year.     
...
applies the VAST (Thorson et al. 2015) approach and due to allowing for
...
meta-analyses on priors for the steepness parameter.
...
estimates (and uncertainty) for `r paste0(thisyr,"-",thisyr+2)` (Table \ref{tab:wtage}).
The changes in weight-at-age in the fishery are substantial, especially for the
apparent abundant year-classes (e.g., the 4-9 year-olds from 2012-2017 representing
the 2012 year class; Fig. \ref{fig:fsh_wtage}).) To examine this more closely,
we split the data into area-season strata and were able to get an overall 
picture of the pattern by strata (Fig. \ref{fig:fsh_wtage_comb}).  
Breaking this further by year and strata for recent years showsFig. \ref{fig:fsh_wtage_strata}).  
...
1991-2018). Results of this analysis shows clear differences in season/area
and years (Fig. \ref{fig:fsh_lw_anomaly}).  All three strata show considerable
inter-annual variability about the long-term (all season and sizes) mean
weight given length. The clearest signal in the data appears to be that the anomalies
are consistent across strata but vary by year. That is, when fish are "fatter" 
or skinnier than normal (given length), then it's generally true for each of the 
three spatio-temporal strata (Fig. \ref{fig:fsh_lw_anomaly}). For the recent years
(since 2015) the summer fishery in the SE (east of 170W longitude) has been well 
below average given length though available data for 2018 suggests that the "condition"
for pollock in that region is improving.
data, the fish are 
normal
...
treatments showed different patterns but ended up in strikingly similar model
...
management quantities (Tab. \ref{tab:mgt_quants}).

The fit to the early Japanese fishery CPUE data (Low and Ikeda 1980) is
consistent with the population trends for this period (Fig.
\ref{fig:mod_cpue_fit}).   The model fits the fishery- independent index from the
2006-2018 AVO data well indicating a downward trend since 2016 (Fig.
...
[online](https://goo.gl/8whZ8hXXX) and summary model results are given in
Table \ref{tab:res_summ}.  he code for the model (with dimensions and links to
parameter names) and input files are available upon request.
...
The estimate of $B_{MSY}$ is `r M$bmsys` kt (with a CV of `r round(100*(M$bmsy.sd/M$bmsy),0)`\%) 
which is less than the projected `r nextyr` spawning biomass of `r M$nextyrssbs` kt; 
(Table \ref{tab:res_summ}).  For `r thisyr`, the Tier 1 levels of yield are `r M$maxabc1s` t from a fishable biomass
estimated at around `r M$ABC_biom1s` kt  (Table \ref{tab:tier1_proj}). Estimated
...
(`r M$nextyrssbs` kt) in `r nextyr` and about `r round(M$ssb1/M$bmsy*100,0)`\% 
...
`r thisyr` spawning stock size is about `r round(100*M$dynb0,0)`\%  of the
predicted value had no fishing occurred since 1978 (Table
\ref{tab:res_summ}). This compares with the `r round(100*M$curssb/M$b100,0)`\% 
...
2.  The near-term prognosis that were relatively few juvenile pollock observed in the surveys during the 
...
5.  The multispecies model suggests that the $B_{MSY}$ level is around xx million
...
11. Finally, given the same estimated aggregate fishing effort in 2018, the estimated stock trend is
    ...
    through to 2022 will require about *75\% more effort* with a decline in spawning biomass of about 30\% compared to
...
Level 2 Substantially increased concerns & Substantially increased assessment uncertainty unresolved issues.                                       & Stock trends are unusual; abundance increasing or decreasing faster than has been seen recently, or recruitment pattern is atypical.             & Some indicators showing an adverse signals but the pattern is not consistent across all indicators.                                                            \\
...
**We therefore rated the population-dynamic concern as level 2, a
substantially increased concern.**

*Environmental/Ecosystem considerations*\footnote{This development of 
this consideration was aided by Dr. E. Siddon, pers. comm.}
...
pollock, remains was below the long-term mean with pollock declining 59\%
...
A score of level 2 suggests that setting the ABC below the maximum permissible
is warranted. The SSC recommended against using a table that showed example
alternatives to select buffers based on that risk level. Thompson (unpublished
Sept 2018 plan team document) tabulated the magnitude of buffers applied by
the Plan Teams for the period 2003-2017, and found that the mode of the
buffers recommended was 10-20 percent. Taking this as guideline, we therefore
recommend application of a buffer of 15% to obtain the recommended ABC (in this case,
  $0.85 \times \it{maxpermissible}\text{ABC}$ = 
 `r formatC(M$maxabc1*0.850,big.mark=",",digits=0,format="d")` kt).

In the past, the SSC has considered factors about the stock assessment
and had selected ABC based on Tier 3 estimates. This year, doing so would
imply a buffer of  `r round(100*(1-M$Tier3_ABC1/M$maxabc1),0)`\%.
...
(and require considerably more capacity and effort). Adopting a more stable catch
system would also result in less spawning stock variability.

In addition to these factors, as modified from Ianelli et al. (2012), a set of performance
indicators at were evaluated relative to 2019 catch values (and assuming the same effort for
the four additional projection years). These indicators are summarized in Table \ref{tab:dec_tab_rationale}). 
Results for model 16.1 (Table \ref{tab:dec_table}) show that there is a low probability
that any of the catches shown in the first row would exceed the $F_{MSY}$ level. Also,
in the near term it appears unlikely that the spawning stock will be below $B_{MSY}$ (rows 3 and 4). 
Relative to the historical mean spawning biomass, by 2020 it is more likely than not that
the spawning biomass will be lower than the historical mean (fifth row). The range of catches
examined have relatively small or no impact on  the age diversity indicators. However,
for catch to equal the 2018 value, more fishing effort will likely be required and there
is an good chance that the proportion of the stock less than age 6 will be greater than
...
465 thousand fish but the 2018 level was about a little more than the
2003-2017 average of 227 thousand fish; Table \ref{tab:pscbycatch}). Chinook
salmon bycatch has also increased steadily since 2012 with the 201j counts at
just over 30,000 (which is 18% below the 2003-2017 mean value). In 2018 the
...
We thank the survey staff who always collect samples diligently, especially this year
when extra effort was required to process data due to unforseen problems with vessel operations. 
Dr. E. Siddon is thanked for graciously providing help to
summarize ecosystem considerations which assisted in ABC evaluations.
The AFSC age-and-growth department is thanked for their continued
excellence in promptly processing the samples used in this assessment. 
Finally, thanks to the many colleagues who provided edits and suggestions to improve this document and to 
Jim Thorson for helping with compiling the alternative index from bottom trawl survey data.
