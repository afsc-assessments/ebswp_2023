...
`r round(M$age3plus[45]/1000,2)` million t.
...
`r round(100*M$curssb/M$bmsy,0)`\% of $B_{MSY}$ (`r M$bmsys` kt). 
The replacement
...
variability would be extremely high (and unrealistic give current capacity and OY limits for combined
 ...
 Yield) noting that a typical target would be 1.2$/times B_{MSY}$. In this case that would make the female spawning biomass target at `r round(1.2*M$bmsy/1000,3) ` million t. It therefore 
...
were developed to evaluate some near-term risks given  alternative `r nextyr` catch
...
fixed `r nextyr` catch and assumes the same effort  for the four additional
projection years (`r nextyr+1`--`r nextyr+4`).  Given this specification , there is a low
...
spawning biomass, by `r nextyr` it is more likely than not that the spawning biomass
...
roughly even chance that the stock in  `r nextyr+1` will be equal to the long term mean.
