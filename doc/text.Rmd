...
$F_{ABC}$                             &	0.336       &	0.336       &		`r (M$fabcs)`           &	`r (M$fabcs)`        \\
...
appear to move to the southeast occurs as they age (Buckley et al. 2009).
...
Nunivak Island to the Russian convention line and into Norton Sound) from 2017 and 2018, as shown
...
(Amendment 91 to the BSAI Groundfish Fishery Management Plan (FMP) resulting from the NPFMC’s 2009 action).
...
[online](https://goo.gl/8whZ8hXXX). The code for the model (with dimensions
...
the $F_{40\%}$ harvest rate as the an FABC value and use $F_{35\%}$ as a
...
2.  The near-term prognosis that there were relatively few juvenile pollock observed in the surveys during the 
...
period 2003--2017, and found that the mode of the buffers recommended was 10--
20 percent. Taking this as guideline, we therefore recommend application of a
buffer of 15% to obtain the recommended ABC 
(in this case, $0.85 \times \text{ABC}_{max}$ = `r formatC(M$maxabc1*0.850,big.mark=",",digits=0,format="d")` kt).

In the past, the SSC has considered factors about the stock assessment and had
selected ABC based on Tier 3 estimates. This year, doing so would imply a
buffer of  `r round(100*(1-M$Tier3_ABC1/M$maxabc1),0)`\%. Recognizing that the
actual catch will be constrained by other factors (the 2 million t BSAI
...
should another period of poor recruitments occur).  To enhance these
...
advice, aiming for  a catch between 1.25 and 1.374 million t results in a even
chance that the stock in  2020 will be equal to the long term mean.
