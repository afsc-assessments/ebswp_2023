  ...
  for (int k=1;k<=nscen;k++)
  {
    ...
    if (k==22)
      ftmp = F(endyr_r,6); //  sel_fut/=sel_fut(6); // NORMALIZE TO AGE 6
			...
			if (nscen>7)
        ftmp= F(endyr_r,6) * ((double(k)-1.)*.05 + 0.5); // this takes endyr F and brackets it...
	if (nscen==7)
	{
    switch (k)
    {
...
void model_parameters::write_R(void)
{
 ...
 // get spr rates for ABC and OFL
  ...
  dvariable OFL  = gm_b(1)*am_f*adj_1(1); 
	...
	{
    report << endyr_r+1<<" " << ABC <<" " << OFL <<" "<< future_SSB(4,styr_fut) <<" "<< age_3_plus_biom(endyr_r+1) <<" "<<
    future_catch(11,styr_fut) <<" " << hm_f  <<" "<< am_f <<" "<< gm_b(1) <<" "<< SPR_ABC <<" "<< SPR_OFL <<endl;
	}
