...
```{r run,eval=FALSE}
  mod_dir   <- c("avon2","ssb0","ssb1","ssb2")
  mod_names <- c("avon2","ssb0","ssb1","ssb2")
  # Need to make output=TRUE default, also check on spm
  ssbres <- run_model(Output=TRUE)
  tab_fit(ssbres, mod_scen = c(1:4)) |> gt::gt()
  tab_ref(ssbres, mod_scen = c(1:4)) |> gt::gt()
