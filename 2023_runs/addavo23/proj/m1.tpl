...
FUNCTION Run_Sim
  detail_out<<"Stock Alt Sim Yr  SSB Rec Tot_biom SPR_Implied F Ntot Catch ABC OFL AvgAge AvgAgeTot SexRatio FABC FOFL"<<endl;
    for (int ispp=1;ispp<=nspp;ispp++)
    {
     ...
     for (int ispp=1;ispp<=nspp;ispp++) 
       detail_out     <<      spname(ispp) 
                      ...
                      <<" "<<SRsim(ispp,isim,ipro)
											<<" "<< FABCs_by_yr(ipro,ispp)
											<<" "<<FOFLs_by_yr(ipro,ispp)
											<< endl;
   } // Loop over projection years
