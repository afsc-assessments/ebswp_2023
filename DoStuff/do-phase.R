...
sdf <- tibble(data.frame(year=1964:2023,rbind(M$sel_fsh,M$sel_fut)))
tail(sdf)
...
wtdf
 ...
 p1
  ggsave("figs/fmsy_sel.pdf",plot=p1,width=5.2,height=4.0,units="in")
ndf <- tibble(data.frame(year=1964:2022,rbind(M$N)))
...
sdfm
 ...
 theme_few() + geom_path(size=.2,color="grey") + xlab( "Mean age in stock") + geom_smooth(method='lm');p1
  ...
  yr=2001:2021;sel<-M$sel_fsh[(nrows-20):nrows,1:12]
